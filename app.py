...
if option=='API認証とデータ取得':
    ...
    if uploaded_file is not None:
        ...
        st.write('認証が完了しました')
        username = st.text_input('ユーザー名を入力してください。', 'junsasakimdt')
...
if option=='データ分析':
    ...
    if uploaded_file is not None:
        ...
        hist1 = alt.Chart(df, title='いいね数の傾向').mark_bar().encode(
            alt.X('いいね', bin=alt.Bin(extent=[0, df['いいね'].max()], step=10), title='いいね数'),
        ...
        df.loc[df['いいね'] >= 1000, 'いいね評価'] = 'A'
        df.loc[(df['いいね'] < 1000) & (df['いいね'] >= 500), 'いいね評価'] = 'B'
        df.loc[(df['いいね'] < 500) & (df['いいね'] >= 300), 'いいね評価'] = 'C'
        df.loc[(df['いいね'] < 300) & (df['いいね'] >= 100), 'いいね評価'] = 'D'
        df.loc[df['いいね'] < 100, 'いいね評価'] = 'E'
